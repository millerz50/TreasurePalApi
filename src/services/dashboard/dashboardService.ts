...
if (!APPWRITE_ENDPOINT || !APPWRITE_PROJECT_ID || !APPWRITE_API_KEY) {
  console.warn(
    "Appwrite configuration incomplete. Ensure APPWRITE_ENDPOINT, APPWRITE_PROJECT_ID and APPWRITE_API_KEY are set."
  );
}
...
async function createDocumentTables(
  ...
  try {
    ...
    console.error(
      `createDocumentTables error for ${collectionId}:`,
      err?.message ?? err
    );
    throw err;
  }
...
export async function getAgentDashboardMetrics(agentId: string) {
  ...
  try {
    ...
    const metricsDocs = metricsRes.documents.filter(
      (d: any) => String(d.agentId) === String(agentId)
    );
...
export async function recordAgentMetrics(agentId: string, metrics: any) {
  ...
  try {
    ...
    const doc = await createDocumentTables(
      DB_ID,
      METRICS_COLLECTION,
      ID.unique(),
      payload
 ...
 */
export async function getUserProfileByAccountId(accountId: string) {
  if (!accountId) throw new Error("accountId is required");

  try {
    const res = await fetchDocumentsTables(DB_ID, USER_TABLE_ID);
    const found = res.documents.find(
      (d: any) => String(d.accountId) === String(accountId)
    );
    return found ?? null;
  } catch (err: any) {
